...
void YCbCrtoRGB(ImgYCbCr **YCbCr, ImgRGB **RGB, int H, int W)
{
    ...
    for (i = 0; i < H; i++)
    {
        for (j = 0; j < W; j++) //用Inverse 的會比較準
        {
            // Red  // 不能直接 (int)  要4捨5入
            int r = round(YCbCr[i][j].Y - 0.000001218894189 * (YCbCr[i][j].Cb - 128) + 1.401999589 * (YCbCr[i][j].Cr - 128));
            r = r > 255 ? 255 : (r < 0 ? 0 : r);
            RGB[i][j].R = (uint8_t)r;

            // Green
            int g = round(YCbCr[i][j].Y - 0.3441356782 * (YCbCr[i][j].Cb - 128) - 0.7141361556 * (YCbCr[i][j].Cr - 128));
            g = g > 255 ? 255 : (g < 0 ? 0 : g);
            RGB[i][j].G = (uint8_t)g;

            // Blue
            int b = round(YCbCr[i][j].Y + 1.772000066 * (YCbCr[i][j].Cb - 128) + 0.0000004062980629 * (YCbCr[i][j].Cr - 128));
            b = b > 255 ? 255 : (b < 0 ? 0 : b);
            RGB[i][j].B = (uint8_t)b;
        }
    }
...
void RLEDecode(RLEData *rleData, int rleCount, short *zigzagged)
{
    ...
    for (int i = 0; i < rleCount; i++)
    {
        ...
        if (rleData[i].value != 0 || j != 0) // 檢查是否結束
        {
...
struct HuffmanNode huffmanTable[MAX_TREE_HT];
int huffmanTableSize = 0;

void readHuffmanTableA(const char *filename)
{
    ...
    char symbol;
    char code[MAX_TREE_HT];
    while (fscanf(file, " %c %s", &symbol, code) == 2)
    {
        huffmanTable[huffmanTableSize].symbol = symbol;
        strcpy(huffmanTable[huffmanTableSize].code, code);
        huffmanTableSize++;
    }
...
char *decodeHuffmanString(const char *encodedStr)
{
    char *decodedStr = malloc(strlen(encodedStr) + 1); // 分配最大可能大小的記憶體
    if (decodedStr == NULL)
    {
        perror("記憶體分配失敗");
        exit(EXIT_FAILURE);
    ...
    {
        tempCode[tempCodeLen++] = encodedStr[i];  //一個字元一個字元加進來
        tempCode[tempCodeLen] = '\0';

        for (int j = 0; j < huffmanTableSize; j++)
        {
            if (strcmp(tempCode, huffmanTable[j].code) == 0)  //配對表直到符合，解碼結果
            {
                decodedStr[decodedPos++] = huffmanTable[j].symbol;
                tempCodeLen = 0; // 重置暫存編碼
                tempCode[0] = '\0';
                break;
            }
        }
    ...
    decodedStr[decodedPos] = '\0'; // 字串結尾
    return decodedStr;
}
void readHuffmanEncodedString(FILE *fp, char **encodedStr) // from ASCII file
{
    // 不知道編碼字符串的準確長度，所以需要動態分配
    size_t size = 1024; // 初始大小
    *encodedStr = malloc(size);
    fgetc(fp); // 跳掉換行
    if (*encodedStr == NULL)
    {
        perror("記憶體分配失敗");
    ...
    while ((ch = fgetc(fp)) != EOF)
    {
        ...
        // 如果當前分配的記憶體不足，則加倍記憶體大小
        if (index >= size)
        {
            size *= 2;
            char *newBuffer = realloc(*encodedStr, size);
            if (newBuffer == NULL)
            {
                free(*encodedStr);
                perror("記憶體重新分配失敗");
    ...
    (*encodedStr)[index] = '\0'; // 字符串結尾
}
int readRLEDataFromString(char *strPtr, RLEData *rleData, int *index)
{
    int rleCount = 0;

    while (strPtr[*index] != '\0')
    {
        short zeroCount, value;

        if (sscanf(strPtr + *index, "%hdn%hdn", &zeroCount, &value) != 2)
        {
            // 無法成功讀取則跳出
            break;
        }

        // 指標移動到第一組數字後 '12n2n4n ->  12n'2n4n
        while (strPtr[*index] != 'n' && strPtr[*index] != '\0')
        {
            (*index)++;
        }
        ...
        if (zeroCount == 0 && value == 0) // 讀到 0n0n則退出   表示讀完一個block
        {
            // 遇到结束標記，跳出
            break;
        }

        rleData[rleCount].zeroCount = zeroCount;
        rleData[rleCount].value = value;
        rleCount++;
    }
    // 更新指標位置
    return rleCount; // 返回RLE Pair數
}
void readBitsFromBinary(FILE *file, int paddedBitCount, char **encodedStr)
{
    unsigned char buffer;
    int totalBitsRead = 0;
    int size = 1024; // 初始大小

    *encodedStr = malloc(size * sizeof(char));
    if (*encodedStr == NULL)
    {
    ...
    while (fread(&buffer, 1, 1, file))  //讀入一個 byte
    {
        int curRead = 0;
        for (int i = 7; i >= 0; --i)  //拆解成bit
        {
            if (++curRead == (8 - paddedBitCount) && feof(file))
            {
                // 如果讀到最後一個 bytes  並且把padding以外的讀完了 就結束
                break;
            }
            if (totalBitsRead >= size) // 動態增大字串記憶體
            {
                size *= 2;
                char *newBuffer = realloc(*encodedStr, size);
                if (newBuffer == NULL)
            ...
            (*encodedStr)[totalBitsRead++] = '0' + ((buffer >> i) & 1); // 11110001  i=7 => '1'1110001
        }
    }
    (*encodedStr)[totalBitsRead] = '\0'; // 字串終止
}
void decoder3(char *fn_bmp, char *type, char *fn_Codebook, char *fn_Huffman_code)
{
    ...
    readHuffmanTableA(fn_Codebook);

    int W, H, NewW, NewH, block_H, block_W, paddedBitCount;
    char *encodedStr = NULL;
    if (strcmp(type, "ascii") == 0)
    {
        FILE *fp_in = fopen(fn_Huffman_code, "r");
        fscanf(fp_in, "%d %d %d %d", &W, &H, &NewW, &NewH); // read size

        // read data

        readHuffmanEncodedString(fp_in, &encodedStr); // 讀取 Huffman 編碼字符串 1010101001111....
    }
    else if (strcmp(type, "binary") == 0)
    {
        FILE *fp_in = fopen(fn_Huffman_code, "rb");
        // 先讀入5個必要資訊 W H newW newH paddedBit
        fread(&W, sizeof(int), 1, fp_in);
        fread(&H, sizeof(int), 1, fp_in);
        fread(&NewW, sizeof(int), 1, fp_in);
        fread(&NewH, sizeof(int), 1, fp_in);
        fread(&paddedBitCount, sizeof(int), 1, fp_in);
        // read data
        readBitsFromBinary(fp_in, paddedBitCount, &encodedStr);
    }

    FILE *fp2 = fopen("test.txt", "w");
    int binSize = strlen(encodedStr);
    char *decodedStr = decodeHuffmanString(encodedStr);
    fprintf(fp2, "%s", decodedStr); 
    block_H = H / 8;
    if (H % 8 != 0)
        block_H++;
    block_W = W / 8;
    ...
    int curr_char = 0;
    for (int row = 0; row < block_H; row++) // 解RLE 解Zigzag  還原block  目前資料: 1n2n6n23n-2n0n0n2n......
    {
        for (int col = 0; col < block_W; col++)
        {
            ...
            int decodedRLECount_Y = readRLEDataFromString(decodedStr, decodedRLEData_Y, &curr_char); // 先把1n2n4n0n0n -> (1,2) (4,0)...

            RLEDecode(decodedRLEData_Y, decodedRLECount_Y, decodedZigzag_Y); // 把(1,2) (4,0) 解回一個ZigZaged string
            inverseZigZagOrder(decodedZigzag_Y, qF_Data, row, col, "Y"); // 反Zigzag，放到對應的Block
            // for(int i = 0; i < decodedRLECount_Y; i++){
            //     printf("Y = %d %d\n",decodedRLEData_Y[i].zeroCount,decodedRLEData_Y[i].value);
            // }
            ...
            int decodedRLECount_Cb = readRLEDataFromString(decodedStr, decodedRLEData_Cb, &curr_char);

            RLEDecode(decodedRLEData_Cb, decodedRLECount_Cb, decodedZigzag_Cb);
            inverseZigZagOrder(decodedZigzag_Cb, qF_Data, row, col, "Cb");

            // for(int i = 0; i < decodedRLECount_Cb; i++){
            //     printf("Cb = %d %d\n",decodedRLEData_Cb[i].zeroCount,decodedRLEData_Cb[i].value);
            // }
            ...
            int decodedRLECount_Cr = readRLEDataFromString(decodedStr, decodedRLEData_Cr, &curr_char);

            RLEDecode(decodedRLEData_Cr, decodedRLECount_Cr, decodedZigzag_Cr);
            inverseZigZagOrder(decodedZigzag_Cr, qF_Data, row, col, "Cr");

            // for(int i = 0; i < decodedRLECount_Cr; i++){
            //     printf("Cr = %d %d\n",decodedRLEData_Cr[i].zeroCount,decodedRLEData_Cr[i].value);
            // }
            // 清理分配的内存
            free(decodedRLEData_Y);
            free(decodedZigzag_Y);
    ...
    if(strcmp(type, "binary") == 0)
        printf("Compression ratio: %.2f\n", (float)H * W * 3 / ((float)binSize / 8)); // 8 個 0101才算一個byte
...
int main(int argc, char **argv)
{
    ...
    else if (strcmp(mode, "3") == 0)
    {
        ...
        decoder3(fn_bmp, type, fn_Codebook, fn_Huffman_code);
    }
    else
    {
        printf("Wrong mode \n");
